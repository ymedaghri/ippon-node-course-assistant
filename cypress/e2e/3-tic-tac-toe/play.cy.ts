...
const stubPostNewGame = () =>
  cy.intercept('http://localhost:3000/new-game', { board: [0, 0, 0, 0, 0, 0, 0, 0, 0] });
...
describe('example to-do app', () => {
  ...
  it('Should not show book with invalid ISBN', () => {
    ...
    cy.get(dataSelector('text.winner')).should('contain.text', 'Computer has won')
    cy.get(dataSelector('board.row(0).col(0)')).should('have.class', 'bg-orange-400')
    cy.get(dataSelector('board.row(1).col(0)')).should('have.class', 'bg-orange-400')
    cy.get(dataSelector('board.row(2).col(0)')).should('have.class', 'bg-orange-400')
